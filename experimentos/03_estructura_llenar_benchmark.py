...
from algoritmos.estructuras import llenar_pila, llenar_cola


def measure_time_llenar(cantidad: int, repeticiones: int = 100) -> dict[str, float]:
    """
    Mide el tiempo total para llenar pilas y colas.

    Args:
        cantidad: Cantidad de elementos a agregar
...
def benchmark():
    """
    Ejecuta el benchmark midiendo tiempo de llenar pilas y colas vs cantidad de elementos.
    """
    print("Iniciando benchmark de llenar Pila y Cola...")
    ...
    for i, cantidad in enumerate(cantidades):
        ...
        tiempos = measure_time_llenar(cantidad, repeticiones=100)
    ...
    plt.title('Benchmark: Llenar Pila vs Cola - Tiempo vs Cantidad de Elementos', fontsize=14, fontweight='bold')
    plt.legend()
    ...
    # Guardar figura
    img_path = os.path.join(os.path.dirname(__file__), 'img', 'estructura_llenar_benchmark.png')
